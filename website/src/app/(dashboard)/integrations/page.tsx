...
function IntegrationCard({
  name, tagline, colorClass, borderClass, icon,
  children, badge,
}: {
...
export default function IntegrationsPage() {
  const { connectHubspot, connectPipedrive } = useAppState();
  ...
  const handleConnectHubspot = (e: React.FormEvent) => {
    ...
    alert("HubSpot integration requires a backend API — coming soon.");
  };

  const handleConnectPipedrive = (e: React.FormEvent) => {
    e.preventDefault();
    if (!pipedriveToken) return;
    alert("Pipedrive integration requires a backend API — coming soon.");
  };

  const handleConnectClay = (e: React.FormEvent) => {
    e.preventDefault();
    if (!clayKey) return;
    alert("Clay integration requires a backend API — coming soon.");
  };

  const handleConnectInstantly = (e: React.FormEvent) => {
    e.preventDefault();
    if (!instantlyKey) return;
    alert("Instantly integration requires a backend API — coming soon.");
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="rounded-2xl border border-white/5 bg-black/20 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 md:gap-4">
          ...
          {/* Step 2 — Clay */}
          <div className="flex flex-col items-center">
            <div className="w-full rounded-2xl border border-white/10 bg-black/40 p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-11 w-11 rounded-xl border border-white/10 bg-white/5 flex items-center justify-center">
                    <Database className="h-5 w-5 text-gray-500" />
                  </div>
                  <div>
                    <p className="text-base font-extrabold text-gray-300">Clay</p>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Data Enrichment</p>
                  </div>
                </div>
                <span className="text-[9px] font-bold px-2 py-1 rounded-full border border-white/10 text-gray-500 uppercase tracking-wider">Step 2</span>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed">
              ...
              <ul className="space-y-1.5">
                {["Waterfall enrichment (150+ data sources)", "Verified email & phone numbers", "AI research agent (Claygent) for personalisation", "Custom lead scoring & filtering"].map((b, i) => (
                  <li key={i} className="flex items-start gap-2 text-[10px] text-gray-500">
                    <Check className="h-3 w-3 shrink-0 mt-0.5 text-gray-600" />{b}
                  </li>
                ))}
              </ul>
            ...
            <div className="w-full rounded-2xl border border-white/10 bg-black/40 p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-base font-extrabold text-gray-300">Instantly.ai</p>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Email Scale & Deliverability</p>
                  </div>
                </div>
                <span className="text-[9px] font-bold px-2 py-1 rounded-full border border-white/10 text-gray-500 uppercase tracking-wider">Step 3</span>
              </div>
              <p className="text-xs text-gray-400 leading-relaxed">
